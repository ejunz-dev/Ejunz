...
const JudgeSettings = Schema.object({
    cache_dir: Schema.string().default(path.resolve(os.homedir(), '.cache', 'ejun', 'judge')),
    tmp_dir: Schema.string().default(path.resolve(os.tmpdir(), 'ejun', 'judge')),
...
export const getConfig: <K extends keyof typeof config>(key: K) => typeof config[K] = global.Ejunz
    ? (key) => global.Ejunz.model.system.get(`ejunzjudge.${key}`) ?? config[key]
    : (key) => config[key];
