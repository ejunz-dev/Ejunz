...
import {
    ...
    enqueueEmbeddingIndex as queueEmbeddingIndex,
    failEmbeddingIndex,
    recoverExpiredEmbeddingWork,
    setEmbeddingProgress,
    EMBEDDING_INDEX_SUBTYPE,
    EMBEDDING_TASK_TYPE,
    type EmbeddingIndexEnqueueInput,
...
export class EmbeddingService extends Service {
    ...
    constructor(ctx: Context) {
        ...
        ctx.on('base/content-change', async (payload: EmbeddingIndexEnqueueInput) => {
            try {
                await queueEmbeddingIndex(payload);
            } catch (err) {
                logger.error('Failed to queue content index change: %o', err);
            }
        });
        ...
        if (process.env.EJUNZ_CLI !== 'true'
            ...
            void recoverExpiredEmbeddingWork().catch((err) => logger.warn('embedding recover failed: %o', err));
            task.consume(
                { type: EMBEDDING_TASK_TYPE, subType: EMBEDDING_INDEX_SUBTYPE },
                async (raw) => {
                    const payload = buildEmbeddingIndexTaskFromDb(raw);
                    const lease = await acquireEmbeddingLease(payload);
                    if (!lease) return;
                    try {
                        await this.processIndexTask(payload);
                        await completeEmbeddingIndex(payload);
                    } catch (error) {
                        await failEmbeddingIndex(payload, error);
                        logger.error('Embedding task failed: %o', error);
                    }
                },
                false,
            );
