...
export const HostSettings = Schema.object({
    ...
    taskTypes: Schema.array(Schema.string()).default(['agent_task', 'tool_call', 'mcp_tool_call', 'embedding_index']).description('Task types this worker can execute'),
});
...
function withGlobalDefaults(base: any) {
    ...
    for (const [name, host] of Object.entries(out.hosts) as [string, any][]) {
        ...
        host.taskTypes ||= ['agent_task', 'tool_call', 'mcp_tool_call', 'embedding_index'];
    }
    return out;
}
