...
export async function findFile(pathname: string, doThrow = true) {
    ...
    if (await fs.pathExists(path.resolve(os.homedir(), '.ejunz', pathname))) return path.resolve(os.homedir(), '.ejunz', pathname);
    // eslint-disable-next-line max-len
    if (await fs.pathExists(path.resolve(os.homedir(), '.config', 'ejunz', pathname))) return path.resolve(os.homedir(), '.config', 'ejunz', pathname);
