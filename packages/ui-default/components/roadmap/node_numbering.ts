...
/**
 * Compute display numbers for roadmap main/sub nodes.
 *
 * Main nodes are numbered 1, 2, 3... top-to-bottom along horizontal solid-linked
 * chains. Chains themselves are ordered by their topmost node's Y position.
 *
 * Sub nodes use the manually-entered value stored in `node.data.nodeNumber` if it
 * matches `x.y` (both x and y are digits). Sub nodes without a valid number are
 * left unnumbered.
 */
...
/**
 * Validate that every sub node has a valid `x.y` number.
 * Returns an array of error messages; empty means all sub nodes are valid.
 */
