...
async function downloadHubFilesAsArchive(type, files) {
  const { links, drdoc } = await request.post('', { operation: 'get_links', files, type });
  const targets = [];
  for (const filename of Object.keys(links)) targets.push({ filename, url: links[filename] });
  await download(`${drdoc.docId} ${drdoc.title}.zip`, targets);
}

const page = new NamedPage('hub_files', () => {
  function ensureAndGetSelectedFiles(type) {
    const files = map(
      $(`.hub-files-${type} tbody [data-checkbox-group="${type}"]:checked`),
  ...
  async function handleClickUpload(button) {
    const did = button.getAttribute('data-did');
    const drid = button.getAttribute('data-drid');
    const drrid = button.getAttribute('data-drrid');

    // 构建上传的 endpoint URL
    const endpoint = `/hub/${did}/${drid}/${drrid}/file`;
    ...
    await uploadFiles(endpoint, files, { type: 'file', pjax: true });
  }

  // 绑定事件处理程序
  $(document).on('click', '[data-op="upload"][data-type="replyfile"]', function() {
    handleClickUpload(this);
  });

  async function handleClickDownloadSelected(type) {
    const selectedFiles = ensureAndGetSelectedFiles(type);
    if (selectedFiles === null) return;
    await downloadHubFilesAsArchive(type, selectedFiles);
  }
  ...
  if ($('[name="upload_replyfile"]').length) {
    $(document).on('click', '[name="upload_commentfile"]', () => handleClickUpload('commentfile'));
    $(document).on('click', '[name="upload_replyfile"]', () => handleClickUpload('replyfile'));
