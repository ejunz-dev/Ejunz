...
function LearnSectionEdit({ sections: initialSections, allSections: allSectionsProp = [], dag: dagProp = [], domainId, targetUid, targetUser, currentLearnSectionIndex: initialLearnIndex = null, currentLearnSectionId: initialLearnId = null }: LearnSectionEditProps) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      await request.post(`/d/${domainId}/collect/section/edit`, body);
  ...
  if (allSections.length === 0) {
    return (
      <div style={{
        ...
        <a
          href={`/d/${domainId}/collect/sections`}
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
            ...
            {targetUid && targetUid !== (window as any).UserContext?._id && (
              <a
                href={`/d/${domainId}/collect/section/edit`}
                style={{
                  padding: isMobile ? '10px 16px' : '10px 20px',
                  minHeight: isMobile ? 44 : undefined,
                  display: 'inline-flex',
...
const page = new NamedPage('collectSectionEditPage', async () => {
  try {
    const container = document.getElementById('collect-section-edit-container');
    if (!container) return;
