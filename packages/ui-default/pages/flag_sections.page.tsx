...
function LearnSectionsTree({ sections, dag, domainId, currentSectionId, currentLearnSectionIndex }: LearnSectionsTreeProps) {
  ...
  const getLessonUrl = useCallback((cardId: string) => {
    return `/d/${domainId}/flag/lesson?cardId=${cardId}`;
  }, [domainId]);

  const getNodeLessonUrl = useCallback((nodeId: string) => {
    return `/d/${domainId}/flag/lesson?nodeId=${encodeURIComponent(nodeId)}`;
  }, [domainId]);
  ...
  const sidebarInner = (
    <>
      <div style={{ padding: '0 16px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
        ...
        <a
          href={`/d/${domainId}/flag/section/edit?uid=${(window as any).UserContext?._id ?? ''}`}
          style={{
            fontSize: '12px',
            color: themeStyles.accent,
            textDecoration: 'none',
...
const page = new NamedPage('flagSectionsPage', async () => {
  try {
    const container = document.getElementById('flag-sections-container');
    if (!container) return;
    ...
    console.error('Failed to render flag sections page:', error);
  }
});
