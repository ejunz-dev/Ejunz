...
function LessonPage() {
    ...
    if (item.type === 'card') {
      ...
      if (isAllDomainsMode && allDomainsEntryDomainId && typeof idx === 'number') {
        return (
          <a
            key={`card-${item.id}`}
            href={`/d/${allDomainsEntryDomainId}/flag/lesson?allDomains=1&cardIndex=${idx}`}
  ...
  const handlePass = async () => {
    ...
    try {
      ...
      const result = await request.post(`/d/${domainId}/flag/lesson/pass`, {
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
            {!isAlonePractice && (
              <button
                onClick={() => {
                  window.location.href = `/d/${domainId}/flag/lesson`;
                }}
                style={{
                  padding: '12px 32px',
                  border: 'none',
            ...
            <button
              onClick={() => {
                window.location.href = `/d/${domainId}/flag`;
              }}
              style={{
                padding: '12px 32px',
                border: 'none',
...
const page = new NamedPage('flagLessonPage', async () => {
  try {
    const container = document.getElementById('flag-lesson-container');
    if (!container) {
      return;
    }
