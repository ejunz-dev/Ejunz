...
function ensureAndGetSelectedAids() {
  const aids = _.map(
    $('tbody [data-checkbox-group="agent"]:checked'),
    (ch) => $(ch).closest('tr').attr('data-aid'),
  );
  if (aids.length === 0) {
    Notification.error(i18n('Please select at least one agent to perform this operation.'));
    return null;
  }
  return aids;
}
...
async function handleOperation(operation) {
  const aids = ensureAndGetSelectedAids();
  if (aids === null) return;
  const payload: any = {};
  if (operation === 'delete') {
    const action = await new ConfirmDialog({
      $body: tpl.typoMsg(i18n('Confirm to delete the selected agents?')),
    }).open();
    if (action !== 'yes') return;
  } else if (operation === 'copy') {
    $(tpl`
      <div style="display: none" class="dialog__body--agent-copy">
        <div class="row"><div class="columns">
          <h1 name="select_user_hint">${i18n('Copy Agents')}</h1>
        </div></div>
        <div class="row">
          <div class="columns">
            <label>
    ...
    const domainSelector: any = DomainSelectAutoComplete.getOrConstruct($('.dialog__body--agent-copy [name="target"]'));
    const copyDialog = await new ActionDialog({
      $body: $('.dialog__body--agent-copy > div'),
      onDispatch(action) {
  ...
  try {
    await request.post('', { operation, aids, ...payload });
    Notification.success(i18n(`Selected agents have been ${operation === 'copy' ? 'copie' : operation}d.`));
    await delay(2000);
    loadQuery();
  } catch (error) {
...
function hideTags(target) {
  for (const i of $(target).find('.agent__tag').get()) {
...
async function handleDownload(ev) {
  ...
  const aids = ensureAndGetSelectedAids();
  if (aids) await downloadProblemSet(aids, name);
}
...
const page = new NamedPage('agent_domain', () => {
  const $body = $('body');
  ...
  for (const op of ['delete', 'hide', 'unhide', 'copy']) {
    $(document).on('click', `[name="${op}_selected_agents"]`, () => handleOperation(op));
  }
  $(document).on('click', '[name="download_selected_agents"]', handleDownload);

  $(document).on('click', '.toggle-tag', () => {
    $('.section__table-container').children().toggleClass('hide-agent-tag');
  });
