...
function LearnSectionsTree({ sections, dag, domainId, currentSectionId, currentLearnSectionIndex }: LearnSectionsTreeProps) {
  ...
  const getLessonUrl = useCallback((cardId: string) => {
    return `/d/${domainId}/collect/lesson?cardId=${cardId}`;
  }, [domainId]);

  const getNodeLessonUrl = useCallback((nodeId: string) => {
    return `/d/${domainId}/collect/lesson?nodeId=${encodeURIComponent(nodeId)}`;
  }, [domainId]);
  ...
  const sidebarInner = (
    <>
      <div style={{ padding: '0 16px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
        ...
        <a
          href={`/d/${domainId}/collect/section/edit?uid=${(window as any).UserContext?._id ?? ''}`}
          style={{
            fontSize: '12px',
            color: themeStyles.accent,
            textDecoration: 'none',
...
const page = new NamedPage('collectSectionsPage', async () => {
  try {
    const container = document.getElementById('collect-sections-container');
    if (!container) return;
