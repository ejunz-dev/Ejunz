...
import * as status from '@ejunz/common/status';
import { findFileSync, getAlphabeticId } from '@ejunz/utils/lib/utils';
import {
...
class Nunjucks extends nunjucks.Environment {
  constructor(Loader: any) {
    ...
    this.addGlobal('model', global.Ejunz.model);
    this.addGlobal('lib', { difficulty: difficultyAlgorithm });
    this.addGlobal('ui', global.Ejunz.ui);
...
export class TemplateService extends Service {
  ...
  constructor(public ctx: Context) {
    ...
    ctx.on('handler/create', async (h) => {
      h.user = h.context.EjunzContext.user as any;
