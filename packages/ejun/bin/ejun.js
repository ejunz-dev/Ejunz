...
const { bypass } = hook(/^(ejun|@ejunz|cordis|lodash|js-yaml)($|\/)/, (id) => {
    if (id.startsWith('ejunz/src')) {
    ...
    return bypass(() => {
        try {
            ...
            try {
                ...
                try {
                    return require.resolve(id.replace(/^@ejunz\//, './'), { paths: [`${homedir()}/.ejunz/addons`] });
                } catch (er) {
                    return id;
                }
            }
