...
export async function load() {
    ...
    if (!process.stdin.isTTY) {
        console.error('Please fill in ~/.ejunz/config.json with MongoDB connection string in the following format:');
        console.error('{"url":"mongodb://localhost:27017/ejunz"}');
        process.exit(1);
    }
    // eslint-disable-next-line no-constant-condition
    while (true) {
        console.error('Please type your MongoDB connection string below:');
        console.error('Example: mongodb://localhost:27017/ejunz');
        try {
            ...
            fs.ensureDirSync(path.resolve(os.homedir(), '.ejunz'));
            fs.writeFileSync(path.resolve(os.homedir(), '.ejunz', 'config.json'), JSON.stringify({ url: uri }));
            break;
        } catch (e) {
            console.error(e);
        }
