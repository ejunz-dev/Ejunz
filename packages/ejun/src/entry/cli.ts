...
async function runScript(name: string, arg: any) {
    const s = global.Ejunz.script[name];
...
async function cli() {
    ...
    if (!global.Ejunz.model[modelName]) {
        return console.error(`Model ${modelName} doesn't exist.`);
    }
    if (!func) {
        return console.log(Object.keys(global.Ejunz.model[modelName]));
    }
    if (!global.Ejunz.model[modelName][func]) {
        return console.error(`Function ${func} doesn't exist in model ${modelName}.`);
    }
    if (typeof global.Ejunz.model[modelName][func] !== 'function') {
        return console.error(`${func} in model ${modelName} is not a function.`);
    }
    const parameterMin = global.Ejunz.model[modelName][func].length;
    const parameters = parseParameters(global.Ejunz.model[modelName][func]);
    ...
    let result = global.Ejunz.model[modelName][func](...args);
