...
export class FileModel {
    ...
    static async addReplyFile(
        ...
        if (!Hubfileinfo) {
            await FileModel.push(domainId, drrid, 'replyfile', { _id: name, ...payload });
        } else {
            await document.setSub(domainId, document.TYPE_HUB_REPLY, drrid, 'replyfile', name, payload);
        }
    }
    ...
    static push<T extends ArrayKeys<HubTailReplyDoc>>(domainId: string, _id: ObjectId, key: ArrayKeys<HubTailReplyDoc>, value: HubTailReplyDoc[T][0]) {
        return document.pushwithid(domainId, document.TYPE_HUB_REPLY, _id, key, value);
    }
}
