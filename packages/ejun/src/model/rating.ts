...
import { coll as documentColl, getMulti, TYPE_BASE, TYPE_CARD, TYPE_NODE } from './document';
...
export async function aggregateContributionByUser(domainId: string): Promise<DomainStatRow[]> {
    ...
    const nodeAgg = await documentColl.aggregate<{ _id: number; c: number }>([
    ...
    const cardAgg = await documentColl.aggregate<{ _id: number; cards: number; problems: number }>([
    ...
    const bases = await documentColl
        .find(
