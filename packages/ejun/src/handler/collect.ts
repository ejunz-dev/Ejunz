...
class CollectEditHandler extends Handler {
    async after(domainId: string) {
        if (this.request.json || !this.response.template) return;
    ...
    async get(domainId: string) {
        ...
        this.response.template = 'collect_edit.html';
        this.response.body = {
            domainId,
            branches,
            currentBranch,
...
class CollectLessonHandler extends Handler {
    async after(domainId: string) {
        if (this.request.json || !this.response.template) return;
    ...
    async get(domainId: string, resultId?: ObjectId) {
        ...
        if (!queryCardId && !queryNodeId && !queryToday) {
            ...
            if (lessonMode === 'allDomains') {
                this.response.redirect = `/d/${finalDomainId}/collect/lesson?allDomains=1&cardIndex=${Math.max(0, lessonCardIndex)}`;
                return;
            }
            if (lessonMode === 'today') {
                this.response.redirect = `/d/${finalDomainId}/collect/lesson?today=1&cardIndex=${Math.max(0, lessonCardIndex)}`;
                return;
            }
            if (lessonMode === 'node' && lessonNodeId) {
                this.response.redirect = `/d/${finalDomainId}/collect/lesson?nodeId=${encodeURIComponent(lessonNodeId)}&cardIndex=${Math.max(0, lessonCardIndex)}`;
                return;
            }
            this.response.redirect = `/d/${finalDomainId}/collect/lesson?today=1`;
            return;
        }
        ...
        if (queryCardId) {
            ...
            this.response.template = 'collect_lesson.html';
            this.response.body = {
                card,
        ...
        if (queryNodeId) {
            ...
            if (reviewCardId) {
                ...
                if (fromReview) {
                    ...
                    await touchLessonSession(finalDomainId, this.user._id, { appRoute: 'collect', lessonReviewCardIds, lessonCardTimesMs }, { silent: true });
                } else {
        ...
        if (queryToday) {
            ...
            const dailyGoalToday = Math.max(0, getModeDailyGoal(dudocToday as any, 'collect'));
    ...
    async getAllDomainsLesson(entryDomainId: string) {
        ...
        if (!currentCard || !currentCard.problems || currentCard.problems.length === 0) {
            this.response.redirect = `/d/${entryDomainId}/collect/lesson?allDomains=1&cardIndex=0`;
            return;
        }
    ...
    async postPassAllDomains(domainId: string, body: any) {
        ...
        if (!base) {
            this.response.body = { success: true, redirect: `/d/${entryDomainId}/collect/lesson?allDomains=1&cardIndex=0` };
            return;
        }
        ...
        if (noImpression) {
            ...
            await touchLessonSession(currentCardDomainId, this.user._id, { appRoute: 'collect', lessonReviewCardIds: reviewIds }, { silent: true });
        } else {
        ...
        this.response.body = { success: true, redirect: `/d/${entryDomainId}/collect/lesson?allDomains=1&cardIndex=${nextIndex}` };
    }
    ...
    async postPass(domainId: string) {
        ...
        if (isTodayMode) {
            ...
            if (!currentCardId) {
                this.response.body = { success: true, redirect: `/d/${finalDomainId}/collect` };
                return;
            }
            ...
            if (nextIndex < cardsForToday.length) {
                this.response.body = { success: true, redirect: `/d/${finalDomainId}/collect/lesson?today=1&cardIndex=${nextIndex}` };
            } else {
                this.response.body = { success: true, redirect: `/d/${finalDomainId}/collect` };
            }
            return;
        ...
        if (nodeIdFromBody) {
            ...
            if (noImpression) {
                ...
                if (!reviewIds.includes(currentCardId.toString())) reviewIds.push(currentCardId.toString());
                await touchLessonSession(finalDomainId, this.user._id, { appRoute: 'collect', lessonReviewCardIds: reviewIds, lessonCardTimesMs: timesMs }, { silent: true });
                ...
                await touchLessonSession(finalDomainId, this.user._id, { appRoute: 'collect', lessonReviewCardIds: nextReviewIds, lessonCardTimesMs: timesMs }, { silent: true });
            } else {
                ...
                await touchLessonSession(finalDomainId, this.user._id, { appRoute: 'collect', lessonReviewCardIds: nextReviewIdsKnow, lessonCardTimesMs: timesMs }, { silent: true });
            }
            const nextIndex = cardIndexFromBody + 1;
            if (nextIndex < flatCardsRaw.length) {
                this.response.body = { success: true, redirect: `/d/${finalDomainId}/collect/lesson?nodeId=${encodeURIComponent(nodeIdFromBody)}&cardIndex=${nextIndex}` };
                return;
            }
            ...
            if (reviewIds2.length > 0) {
                this.response.body = { success: true, redirect: `/d/${finalDomainId}/collect/lesson?nodeId=${encodeURIComponent(nodeIdFromBody)}&reviewCardId=${encodeURIComponent(reviewIds2[0])}` };
            } else {
                await touchLessonSession(finalDomainId, this.user._id, {
                    appRoute: 'collect',
                    lessonMode: null,
                ...
                this.response.body = { success: true, redirect: `/d/${finalDomainId}/collect/lesson/node-result?nodeId=${encodeURIComponent(nodeIdFromBody)}` };
            }
            return;
        }
        ...
        if (isBrowseOnly && noImpressionBody) {
            ...
            this.response.body = { success: true, redirect: `/d/${finalDomainId}/collect/lesson?cardId=${cardIdStr}&reviewCardId=${encodeURIComponent(cardIdStr)}` };
            return;
        }
        ...
        if (isAlonePractice) {
            ...
            if (nextReviewIds.length !== reviewIdsAfter.length) {
                await touchLessonSession(finalDomainId, this.user._id, { appRoute: 'collect', lessonReviewCardIds: nextReviewIds }, { silent: true });
            }
        }

        this.response.body = { success: true, redirect: `/d/${finalDomainId}/collect/lesson/result/${resultId}` };
    }
    ...
    async getResult(domainId: string, resultId: ObjectId) {
        ...
        this.response.template = 'collect_lesson_result.html';
        this.response.body = {
            card,
            node,
            problemStats,
...
class CollectLessonNodeResultHandler extends Handler {
    async after(domainId: string) {
        if (this.request.json || !this.response.template) return;
        this.response.body.overrideNav = [
            { name: 'homepage', args: {}, displayName: this.translate('Home'), checker: () => true },
            { name: 'collect', args: {}, displayName: this.translate('Learn'), checker: () => true },
        ];
    }
    ...
    async get(domainId: string) {
        ...
        this.response.template = 'collect_lesson_node_result.html';
        this.response.body = {
            domainId: finalDomainId,
            baseDocId: base.docId.toString(),
            rootNodeTitle: rootNode.title || '',
...
class CollectBaseSelectHandler extends Handler {
    async get(domainId: string) {
        ...
        const redirect = typeof this.request.query?.redirect === 'string' && this.request.query.redirect
            ...
            : `/d/${finalDomainId}/collect`;
        this.response.template = 'collect_base_select.html';
        this.response.body = {
            domainId: finalDomainId,
            bases,
            selectedBaseDocId,
...
export async function apply(ctx: Context) {
    ...
    ctx.Route('collect_lesson_pass', '/collect/lesson/pass', CollectLessonHandler);
    ctx.Route('collect_lesson_node_result', '/collect/lesson/node-result', CollectLessonNodeResultHandler);
}
