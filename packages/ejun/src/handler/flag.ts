...
const FLAG_DAILY_GOAL_MAX = 10000;

function flagUtcDayKey(): string {
    return moment.utc().format('YYYY-MM-DD');
}

function mergeFlagCheckinNodeProgress(
    dudoc: Record<string, unknown> | null | undefined,
    newNodeIds: string[],
): Record<string, unknown> {
    const today = flagUtcDayKey();
    let date = typeof dudoc?.flagProgressDate === 'string' ? (dudoc.flagProgressDate as string) : '';
    let ids: string[] = Array.isArray(dudoc?.flagProgressNodeIds)
        ? (dudoc.flagProgressNodeIds as unknown[]).map((x) => String(x))
        : [];
    const activityDates: string[] = Array.isArray(dudoc?.flagActivityDates)
        ? [...((dudoc.flagActivityDates as unknown[]).map((x) => String(x)))]
        : [];
    if (date !== today) {
        date = today;
        ids = [];
    ...
    for (const raw of newNodeIds) {
        const id = String(raw || '').trim();
        if (id && !id.startsWith('temp-node-')) set.add(id);
    }
    ids = Array.from(set);
    ...
    return {
        flagProgressDate: date,
        flagProgressNodeIds: ids,
        flagActivityDates: activityDates,
    };
}

function countConsecutiveFlagDays(activityDates: string[]): number {
...
class FlagHandler extends Handler {
    ...
    async get(domainId: string) {
        ...
        const emptyBody = {
            ...
            learnBases: [] as Array<{ docId: number; title?: string; bid?: string }>,
            selectedLearnBaseDocId: null as number | null,
            requireBaseSelection: false,
            baseDocId: null as string | null,
            dailyGoal: 0,
        ...
        if (initialNodes <= 1 && initialEdges === 0 && branchDataNodes <= 1 && branchDataEdges === 0) {
            ...
            if (reloadedBase) base = reloadedBase;
        }
        ...
        if (dbBase) {
            ...
            if (dbNodes.length > (base.nodes?.length || 0) || dbEdges.length > (base.edges?.length || 0)) {
                base.nodes = dbNodes;
                base.edges = dbEdges;
                if (dbBase.branchData) base.branchData = dbBase.branchData;
            }
        }
...
class FlagEditHandler extends Handler {
    async after(domainId: string) {
        if (this.request.json || !this.response.template) return;
    ...
    async get(domainId: string) {
        ...
        this.response.template = 'flag_edit.html';
        this.response.body = {
            domainId,
            branches,
            currentBranch,
...
class FlagLessonHandler extends Handler {
    async after(domainId: string) {
        if (this.request.json || !this.response.template) return;
    ...
    async get(domainId: string, resultId?: ObjectId) {
        ...
        const queryAllDomains = this.request.query?.allDomains === '1' || this.request.query?.allDomains === 'true';
        ...
        if (!queryCardId && !queryNodeId && !queryToday) {
            ...
            if (lessonMode === 'today') {
                this.response.redirect = `/d/${finalDomainId}/flag/lesson?today=1&cardIndex=${Math.max(0, lessonCardIndex)}`;
                return;
            }
            if (lessonMode === 'node' && lessonNodeId) {
                this.response.redirect = `/d/${finalDomainId}/flag/lesson?nodeId=${encodeURIComponent(lessonNodeId)}&cardIndex=${Math.max(0, lessonCardIndex)}`;
                return;
            }
    ...
    async getAllDomainsLesson(entryDomainId: string) {
        ...
        const allDomainsFlatCards: DomainCardItem[] = [];
        ...
        if (!currentCard || !currentCard.problems || currentCard.problems.length === 0) {
            this.response.redirect = `/d/${entryDomainId}/flag/lesson?allDomains=1&cardIndex=0`;
            return;
        }
    ...
    async postPassAllDomains(domainId: string, body: any) {
        ...
        if (!base) {
            this.response.body = { success: true, redirect: `/d/${entryDomainId}/flag/lesson?allDomains=1&cardIndex=0` };
            return;
        }
        ...
        this.response.body = { success: true, redirect: `/d/${entryDomainId}/flag/lesson?allDomains=1&cardIndex=${nextIndex}` };
    }
    ...
    async postPass(domainId: string) {
        ...
        if (isTodayMode) {
            ...
            if (!currentCardId) {
                this.response.body = { success: true, redirect: `/d/${finalDomainId}/flag` };
                return;
            }
            ...
            const nextIndex = (Number.isNaN(cardIndexFromBody) ? 0 : cardIndexFromBody) + 1;
            if (nextIndex < cardsForToday.length) {
                this.response.body = { success: true, redirect: `/d/${finalDomainId}/flag/lesson?today=1&cardIndex=${nextIndex}` };
            } else {
                this.response.body = { success: true, redirect: `/d/${finalDomainId}/flag` };
            }
            return;
        ...
        this.response.body = { success: true, redirect: `/d/${finalDomainId}/flag/lesson/result/${resultId}` };
    }
    ...
    async getResult(domainId: string, resultId: ObjectId) {
        ...
        this.response.template = 'flag_lesson_result.html';
        this.response.body = {
            card,
            node,
            problemStats,
...
class FlagLessonNodeResultHandler extends Handler {
    async after(domainId: string) {
        if (this.request.json || !this.response.template) return;
        this.response.body.overrideNav = [
            { name: 'homepage', args: {}, displayName: this.translate('Home'), checker: () => true },
            { name: 'flag', args: {}, displayName: this.translate('Learn'), checker: () => true },
        ];
    }
    ...
    async get(domainId: string) {
        ...
        this.response.template = 'flag_lesson_node_result.html';
        this.response.body = {
            domainId: finalDomainId,
            baseDocId: base.docId.toString(),
            rootNodeTitle: rootNode.title || '',
...
class FlagSectionEditHandler extends Handler {
    async after(domainId: string) {
        if (this.request.json || !this.response.template) return;
    ...
    async get(domainId: string) {
        ...
        if (!base) {
            this.response.template = 'flag_section_edit.html';
            this.response.body = {
                sections: [],
                allSections: [],
                dag: [],
...
class FlagSectionsHandler extends Handler {
    async after(domainId: string) {
        if (this.request.json || !this.response.template) return;
    ...
    async get(domainId: string) {
        ...
        if (!base) {
            this.response.template = 'flag_sections.html';
            this.response.body = {
                sections: [],
                domainId: finalDomainId,
                baseDocId: null,
...
/**
 * Reuses base_editor.html + base editor UI with UiContext.editorUiMode = flag_nodes_intent (cards hidden / blocked client-side).
 */
class FlagBaseEditorHandler extends Handler {
    base?: BaseDoc;
    ...
    async get(domainId: string, branch?: string) {
        ...
        if (selectedBaseDocId === null || Number(selectedBaseDocId) !== docIdNum) {
            this.response.redirect = `/d/${finalDomainId}/flag/base/select?redirect=${encodeURIComponent(`/d/${finalDomainId}/flag/base/${docIdNum}/branch/${encodeURIComponent(requestedBranch)}/editor`)}`;
            return;
        }
        ...
        const flagProgressNodeIds =
            fpDate === todayKey && Array.isArray(fpIdsRaw)
                ? fpIdsRaw.map((x: unknown) => String(x)).filter((id: string) => id && !id.startsWith('temp-node-'))
                : [];
        this.response.body = {
...
class FlagEditorShortcutHandler extends Handler {
    @param('docId', Types.String)
    async get(domainId: string, docId: string) {
        this.checkPriv(PRIV.PRIV_USER_PROFILE);
        const finalDomainId = typeof domainId === 'string' ? domainId : (domainId as any)?.domainId || this.args.domainId;
        this.response.redirect = `/d/${finalDomainId}/flag/base/${encodeURIComponent(docId)}/branch/main/editor`;
    }
}

class FlagLearnSyncHandler extends Handler {
    ...
    async postSyncDag(domainId: string, docId: number) {
        ...
        const nodeIds = Array.isArray(body.nodeIds)
            ? body.nodeIds.map((x: unknown) => String(x || '').trim()).filter((id: string) => id && !id.startsWith('temp-node-'))
            : [];

        const { selectedBaseDocId } = await getLearnBaseSelection(finalDomainId, this.user._id, this.user.priv);
        if (selectedBaseDocId === null || Number(selectedBaseDocId) !== Number(docId)) {
            throw new ValidationError(this.translate('Select this base in Flag before syncing') || 'Base mismatch');
        }
        ...
        const flagProgressNodeIds =
            merged.flagProgressDate === todayKey && Array.isArray(merged.flagProgressNodeIds)
                ...
                : [];
        this.response.body = { success: true, flagDailyGoal, flagProgressNodeIds };
    }
}

class FlagBaseSelectHandler extends Handler {
    async get(domainId: string) {
        const finalDomainId = typeof domainId === 'string' ? domainId : (domainId as any)?.domainId || this.args.domainId;
        ...
        const redirect = typeof this.request.query?.redirect === 'string' && this.request.query.redirect
            ...
            : `/d/${finalDomainId}/flag`;
        this.response.template = 'flag_base_select.html';
        this.response.body = {
            domainId: finalDomainId,
            bases,
            selectedBaseDocId,
...
export async function apply(ctx: Context) {
    ctx.Route('flag', '/flag', FlagHandler);
    ctx.Route('flag_set_base', '/flag/base', FlagHandler);
    ctx.Route('flag_editor', '/flag/base/:docId/editor', FlagEditorShortcutHandler, PRIV.PRIV_USER_PROFILE);
    ctx.Route('flag_editor_branch', '/flag/base/:docId/branch/:branch/editor', FlagBaseEditorHandler, PRIV.PRIV_USER_PROFILE);
    ctx.Route('flag_learn_sync', '/flag/base/:docId/sync-learn', FlagLearnSyncHandler, PRIV.PRIV_USER_PROFILE);
    ctx.Route('flag_base_select', '/flag/base/select', FlagBaseSelectHandler);
    ctx.Route('flag_set_daily_goal', '/flag/daily-goal', FlagHandler);
    ctx.Route('flag_sections', '/flag/sections', FlagSectionsHandler);
    ctx.Route('flag_section_edit', '/flag/section/edit', FlagSectionEditHandler, PRIV.PRIV_USER_PROFILE);
    ctx.Route('flag_edit', '/flag/edit', FlagEditHandler, PRIV.PRIV_USER_PROFILE);
    ctx.Route('flag_lesson', '/flag/lesson', FlagLessonHandler);
    ctx.Route('flag_lesson_result', '/flag/lesson/result/:resultId', FlagLessonHandler);
    ctx.Route('flag_lesson_pass', '/flag/lesson/pass', FlagLessonHandler);
    ctx.Route('flag_lesson_node_result', '/flag/lesson/node-result', FlagLessonNodeResultHandler);
}
