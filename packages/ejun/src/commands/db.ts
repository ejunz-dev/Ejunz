...
import { Logger, size } from '@ejunz/utils';
...
const ejunzPath = path.resolve(os.homedir(), '.ejunz');
const dir = `${os.tmpdir()}/${Math.random().toString(36).substring(2)}`;
function getUrl() {
    const dbConfig = fs.readFileSync(path.resolve(ejunzPath, 'config.json'), 'utf-8');
...
export function register(cli: CAC) {
    ...
    cli.command('restore <filename>').option('-y', 'Assume yes', { default: false }).action(async (filename) => {
        ...
        exec('mongorestore', [`--uri=${url}`, `--dir=${dir}/dump/ejunz`, '--drop'], { stdio: 'inherit' });
        if (fs.existsSync(`${dir}/file`)) {
            exec('rm', ['-rf', '/data/file/ejunz'], { stdio: 'inherit' });
