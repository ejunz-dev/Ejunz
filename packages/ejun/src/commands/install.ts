...
import tar from 'tar';
import { extractZip, Logger } from '@ejunz/utils';
...
const hydroPath = path.resolve(os.homedir(), '.ejunz');
...
export function register(cli: CAC) {
    cli.command('install [package]').action(async (_src) => {
        ...
        child.execSync(`ejunz addon add '${newAddonPath}'`);
    ...
    cli.command('uninstall [package]').action(async (name) => {
        ...
        if (!plugins.includes(name)) {
            throw new Error(`Plugin ${name} not found or not installed with \`ejunz install\`.`);
        }
        const newAddonPath = path.join(addonDir, name);
        child.execSync(`ejunz addon remove '${newAddonPath}'`, { stdio: 'inherit' });
