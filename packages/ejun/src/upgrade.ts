...
const unsupportedUpgrade = async function _26_27() {
    throw new Error('This upgrade was no longer supported in ejun@4. \
Please use ejun@3 to perform these upgrades before upgrading to v4');
};
...
export const coreScripts: MigrationScript[] = [
    ...
    async function init() {
        if (!await user.getById('system', 0)) {
            await user.create('Guest@Ejunz.local', 'Guest', String.random(32), 0, '127.0.0.1', PRIV.PRIV_REGISTER_USER);
        }
        if (!await user.getById('system', 1)) {
            await user.create('Ejunz@Ejunz.local', 'Ejunz', String.random(32), 1, '127.0.0.1', PRIV.PRIV_USER_PROFILE);
        }
        const ddoc = await domain.get('system');
        if (!ddoc) await domain.add('system', 1, 'Ejunz', 'Welcome to Ejunz!');
        // await welcome();
        return true;
    },
    // Init
    ...
    async function _59_60() {
        const langs = system.get('ejun.langs');
        await system.set('ejun.langs', langs.replace(/\$\{dir\}/g, '/w').replace(/\$\{name\}/g, 'foo'));
        return true;
    },
    async function _60_61() {
        const config = await system.get('ejun.homepage');
        const data = yaml.load(config) as any;
        if (!(data instanceof Array)) {
            await system.set('ejun.homepage', yaml.dump([
    ...
    async function _67_68() {
        const rdocs = RecordModel.coll.find({ code: /^@@Ejunz_submission_file@@/ });
        for await (const rdoc of rdocs) {
            await RecordModel.update(rdoc.domainId, rdoc._id, {
                files: { code: rdoc.code.split('@@Ejunz_submission_file@@')[1] },
                code: '',
            });
        }
        return true;
