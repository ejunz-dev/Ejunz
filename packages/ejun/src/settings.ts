...
const defaultPath = process.env.CI ? '/tmp/file'
    : process.env.DEFAULT_STORE_PATH || '/data/file/ejunz';
const FileSetting = Schema.intersect([
    ...
    Schema.union([
        ...
        Schema.object({
            ...
            bucket: Schema.string().default('ejunz'),
...
export async function loadConfig() {
    const config = await global.Ejunz.service.db.collection('system').findOne({ _id: 'config' });
    try {
...
export async function saveConfig(config: any) {
    ...
    await global.Ejunz.service.db.collection('system').updateOne({ _id: 'config' }, { $set: { value } }, { upsert: true });
    loadConfig();
}
