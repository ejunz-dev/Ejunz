...
export function BaseDetailAiTutor({
  ...
  return (
    <>
      {!open ? (
        <div className="bd-ai-tutor-bar-wrap">
          <button
            type="button"
            className="bd-ai-tutor-bar"
            onClick={handleOpen}
            aria-label={i18n('Roadmap AI tutor open')}
          >
            <span className="bd-ai-tutor-bar__brand">
              <WandIcon />
              {i18n('Roadmap AI tutor')}
            </span>
            <span className="bd-ai-tutor-bar__hint">{i18n('Roadmap AI tutor bar hint')}</span>
          </button>
        </div>
      ) : null}
      ...
      {open ? (
        <>
          <div
            className="bd-ai-tutor-backdrop"
            onClick={() => setOpen(false)}
            aria-hidden
          />
          <div
            className="bd-ai-tutor-modal"
            role="dialog"
            aria-modal="true"
            aria-label={i18n('Roadmap AI tutor')}
          >
            <div className="bd-ai-tutor-modal__header">
              <div className="bd-ai-tutor-modal__title">
                <BookIcon />
                {i18n('Roadmap AI tutor')}
              </div>
              <button
                type="button"
                className="bd-ai-tutor-modal__close"
                onClick={() => setOpen(false)}
            ...
            <div className="bd-ai-tutor-modal__body">
              <div className="bd-ai-tutor-greeting">
                <span className="bd-ai-tutor-greeting__icon" aria-hidden>
                  <SparkleIcon />
                </span>
              ...
              {messages.map((msg, index) => (
                msg.role === 'tool_call' ? (
                  ...
                  <div
                    key={`${index}-${msg.role}`}
                    className="bd-ai-tutor-msg bd-ai-tutor-msg--assistant"
                  >
                    {msg.content
                  ...
                  <div
                    key={`${index}-${msg.role}`}
                    className={`bd-ai-tutor-msg bd-ai-tutor-msg--${msg.role}`}
                  >
                    {msg.content}
                  </div>
                )
              ...
              {showSuggestions ? (
                <div className="bd-ai-tutor-suggestions">
                  <p className="bd-ai-tutor-suggestions__lead">
                    {i18n('Roadmap AI tutor suggestions lead')}
                  </p>
                  <div className="bd-ai-tutor-suggestions__list">
                    {suggestedQuestions.map((question) => (
                      <button
                        key={question}
                        type="button"
                        className="bd-ai-tutor-suggestions__pill"
                        disabled={isLoading}
                        onClick={() => void handleSuggestedClick(question)}
                      >
                        {question}
                      </button>
                    ))}
            ...
            <div className="bd-ai-tutor-modal__footer">
              <div className="bd-ai-tutor-input-wrap">
                <input
                  ref={inputRef}
                  type="text"
                  className="bd-ai-tutor-input"
                  value={input}
                  ...
                  placeholder={String(i18n('Roadmap AI tutor input placeholder'))}
                  disabled={isLoading}
                  aria-label={i18n('Roadmap AI tutor input placeholder')}
                />
                <button
                  type="button"
                  className="bd-ai-tutor-send"
...
export default BaseDetailAiTutor;
