...
export const EditableProblem = React.memo(({
  ...
  const reloadLearnerNotes = useCallback(async () => {
    ...
    try {
      const res: any = await requestJson(`/learn/problem-notes?cardId=${encodeURIComponent(_cardId)}&pid=${encodeURIComponent(problem.pid)}`, { domainId: editorDomainId });
      ...
      setLearnerNoteRows([]);
      learnerNotesBaselineRef.current = [];
      // Learner notes are auxiliary; keep the editor usable when they are unavailable.
    } finally {
...
EditableProblem.displayName = 'BaseDetailProblemForm';
export const BaseDetailProblemForm = EditableProblem;
